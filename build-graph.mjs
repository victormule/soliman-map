 ...
 */
const addEdge = (a, b, type, dir = 0, dashed = false) => {
  if (a === b) return false;
...
for (const c of raw.connectors) {
  ...
  addEdge(a, b, 'connector', dirOf(c.style), (c.style || {}).strokeStyle === 'dotted');
}
...
for (const group of byConcept.values()) {
  for (let i = 0; i < group.length - 1; i++) addEdge(group[i], group[i + 1], 'concept');
}

// 6c. La gravité : un ressort de chaque post-it vers sa section. Elle ne sert
...
// 6d. Degré = ce qui se voit (connecteurs + même texte). La gravité ne compte
